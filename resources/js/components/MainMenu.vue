<template>
  <div class="font-heading font-semibold">
    ...
    <div
      v-if="showInstructorDashboard"
      class="mt-12 mb-4 uppercase text-xs text-gray-400"
    >
      Admin
    </div>
    <div
      v-if="
        $page.auth.user.is_admin ||
          $page.auth.user.is_support ||
          $page.auth.user.is_super
      "
      class="mb-4"
    >
      <inertia-link
    ...
    >
      ...
      >
        <icon
          ...
          class="w-4 h-4 mr-2"
          :class="
            isUrl(prefix, 'courses*', 'course-location')
              ? 'fill-gray-100'
              : 'fill-gray-400 group-hover:fill-gray-100'
          "
        />
    ...
    >
      ...
      >
        <icon
          name="document"
          class="w-4 h-4 mr-2 fill-gray-400 group-hover:fill-gray-100"
        />
...
export default {
  ...
  computed: {
    showInstructorDashboard() {
      return (
        this.$page.auth.user.is_instructor &&
        (this.$page.auth.user.is_admin ||
          this.$page.auth.user.is_support ||
          this.$page.auth.user.is_super)
      )
    },
    prefix() {
      return (
        (this.$page.locale || '') +
        '/' +
        (this.$page.environment === 'production' ? 'admin' : 'touchstone-admin')
      )
    },
  },
  methods: {
